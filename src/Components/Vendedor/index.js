...
import { TOKEN_POST, CLIENT_ID, CLIENT_SECRET, RCA_ALL_POST, FILIAIS_ALL_POST, RECORD_NUMBER_PER_REQUEST } from '../../api/endpoints/geral.js'
...
const Vendedor = ({ defaultFilters, callBakSelectedItem, ignoreTableActions, ...props }) => {
    ...
    const [cancelarVendedor, setCancelarVendedor] = React.useState(false)
    ...
    const handleCodigoVendedorFilter = ({ target }) => {
        setCodigoVendedor(target.value)
    }
    ...
    const filtersArr = [
        {
            ...
            atributsFormControl: { 'type': 'text', size: "sm", 'name': 'id', value: codigoVendedor, onChange: handleCodigoVendedorFilter, onBlur: handleCodigoVendedorFilter, onKeyUp: handleSearch },
    ...
    const acoesBottomCard = [
        {
            ...
            props: { onClick: () => requestAllVendedors(), className: 'btn btn-sm botao_success' }
        },
        {
            label: 'Limpar',
        ...
        },
        {
            ...
            props: { onClick: () => setCadastrarVendedor(true), className: 'btn btn-sm mx-2 btn-secondary' }
        }
    ];
    ...
    React.useEffect(() => {
        switch (acao) {
            ...
            case 'cancelar':
                if (consultaChoice > 0) {
                    setCancelarVendedor(true);
                } else {
                    setCancelarVendedor(false);
                }
                break;
            default:
    ...
    const montarFiltro = () => {
        ...
        if (codigoVendedor) {
            filtros['id'] = codigoVendedor;
            detalhesFiltros['id'] = {
                label: 'Código',
                value: codigoVendedor,
                resetFilter: () => { setCodigoVendedor(''); removeFilter('id') },
            };
        }
    ...
    const requestAllVendedors = async () => {
        setVendedor([])

        let { filtros, detalhesFiltros } = montarFiltro();
        setAppliedFilters(detalhesFiltros)
        let { url, options } = RCA_ALL_POST({ ...filtros }, getToken());
            ...
            <Row>
                            ...
                            <Col xs="12" sm="12" md="12" className={'mobile_card_report pt-4'} style={{ backgroundColor: '#FFF' }}>
                                    ...
                                    <Col className={'mx-2'}  >
                                        <Row style={{ borderRadius: '24px 24px 24px 24px', border: '1px solid #000' }}>
                                            ...
                                            <Col xs="1" sm="1" md="1" style={{ textAlign: 'left', alignItems: 'center', justifyContent: 'center', margin: 'auto', padding: '0' }} >
                                                <FontAwesomeIcon onClick={() => { requestAllVendedors(); }} size={'lg'} icon={faSearch} />
                                            </Col>
                                        </Row>
                                    </Col>
                                </Row>
                                ...
                                <Row>
                                    <div style={{ display: 'flex', flexDirection: 'collumn', flexWrap: 'wrap' }}>
                                        <Button style={{ borderRadius: '50px', marginBottom: '10px', marginRight: '0.4rem' }} className={'btn btn-sm btn-secondary'} onClick={() => { setCadastrarVendedor(true); }} ><FontAwesomeIcon icon={faPlus} /> Vendedor</Button>
                                    </div>
                                </Row>
                            </Col>
                        </>
                ...
                <Col xs="12" sm="12" md={'12'}>
                    <Include
                        dataEstado={estado}
                        loadingData={loading}
                        callBack={requestAllVendedors}
                        requestAllVendedors={requestAllVendedors}
                        setMostarFiltros={setMostarFiltros}
                        idVendedorCriada={consultaChoice}
