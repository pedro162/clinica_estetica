...
const CalendarioSimplesConsultaExterno = ({botoesHeader, rowsTableArr, defaultDate, ...props})=>{
										...
										<tr key={'seaman'+inex+arr.length} >
											{
												[0,1,2,3,4,5, 6].map((it, ind, ar)=>{
													...
													if(defaultDate){
														let objDefaultDate = new Date(defaultDate)
														if(objDefaultDate.getFullYear() == dt.getFullYear() && objDefaultDate.getMonth() == dt.getMonth() && objDefaultDate.getDate() == dt.getDate()){
															//addClass('dia_atual')
															classesEstilos += ' '+estilos.dia_atual
														}
													}
													...
													if(rowsTableArr){
														//if(dtNow.getFullYear() == dt.getFullYear() && dtNow.getMonth() == dt.getMonth()){
															for(let it=0; !(it == rowsTableArr.length); it++){
																...
																let {propsRow, data_format, hora, acoes, dia_semana, celBodyTableArr} = atualItemTable
																...
																if(Array.isArray(data_format_arr) && data_format_arr.length == 3){
																	...
																	if(!(i > getLastDayThisMonth || i < 1)){

																		if(diaSemanaAtual == dia_semana){
																			dadosAgendaData.push(atualItemTable)
																			classesEstilos += ' '+estilos.event
																			classesEstilos += ' '+estilos.background_orange
														...
														//}
																	...
																	//console.log('========================== dados dia ====================')
													let {setDataEscolhida, callback} = propsDate


													i+= 1;
													return(
														<td key={'day'+ind+ar.length}  className={classesEstilos} {...propsDate} onClick={()=>{
															if(setDataEscolhida){
																setDataEscolhida(anoAtualDt+'-'+mesAtualDt+'-'+diaAtualTd);
															}
															//alert(anoAtualDt+'-'+mesAtualDt+'-'+diaAtualTd)
...
export default CalendarioSimplesConsultaExterno;