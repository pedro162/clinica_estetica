...
const Origem = ({idMovimentacoesFinanceira, setIdMovimentacoesFinanceira, callback, atualizarMovimentacoesFinanceira, setAtualizarMovimentacoesFinanceira, setVisualizarOrigemMovimentacoesFinanceira, noUseModal})=>{
			...
			if(idMovimentacoesFinanceira > 0){
				...
				if(json){
					...
					if(Array.isArray(erroValidaao) && erroValidaao.length > 0){
						...
						await setDataMovimentacoesFinanceira(json)
						await setShowModalAtualizarMovimentacoesFinanceira(true)
						await setDefaultFilters({id:data?.referencia_id})
					 }
		        }else{
		        	setDataMovimentacoesFinanceira([])
		        	setVisualizarOrigemMovimentacoesFinanceira(false)
		        }
			}
		}
	...
	if(noUseModal){
		return(
			<>
				...
				dataMovimentacoesFinanceira && <ContasReceber
	...
	if(
		dataMovimentacoesFinanceira && defaultFilters && defaultFilters.hasOwnProperty('id')
	){
		if(dataMovimentacoesFinanceira?.mensagem?.referencia == 'conta_recebers'){
			return(
			<>
				{! dataMovimentacoesFinanceira &&
					<Modal noBtnCancelar={true} noBtnConcluir={true} handleConcluir={()=>null}  title={'Loading...'} 
						size="xs" propsConcluir={{}}
						labelConcluir={''}
						dialogClassName={''}
						aria-labelledby={'aria-labelledby'}
						labelCanelar=""
						show={setShowModalAtualizarMovimentacoesFinanceira}
						showHide={()=>{setShowModalAtualizarMovimentacoesFinanceira();
						setVisualizarOrigemMovimentacoesFinanceira(false)}}
					>
						<Load/>
					</Modal>
				...
				{dataMovimentacoesFinanceira && defaultFilters && defaultFilters.hasOwnProperty('id') && 
					<Modal 
							...
							dialogClassName={'modal-90w'} aria-labelledby={'aria-labelledby'}
							labelCanelar="Fechar"
							show={showModalAtualizarMovimentacoesFinanceira}
							showHide={()=>{setShowModalAtualizarMovimentacoesFinanceira();setIdMovimentacoesFinanceira(null); setVisualizarOrigemMovimentacoesFinanceira(false)}}
	...
	return(
		<>
			<Modal 
					...
					size="lg" propsConcluir={{'disabled':loading}}
					labelConcluir={loading ? 'Salvando...' : 'Concluir'}
					dialogClassName={'xs'} aria-labelledby={'aria-labelledby'}
					labelCanelar="Fechar"
					show={showModalAtualizarMovimentacoesFinanceira}
					showHide={()=>{setShowModalAtualizarMovimentacoesFinanceira();setIdMovimentacoesFinanceira(null); setVisualizarOrigemMovimentacoesFinanceira(false)}}
			...
			>
				...
				<AlertaDismissible
					title="Oops... " message={'Nada encontrado'} variant={'red'} 
				/>
            </Modal>
		</>
	)
