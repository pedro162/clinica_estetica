...
const Iniciar = ({ idOrdemServico, setIdOrdemServico, callback, atualizarOrdemServico, setAtualizarOrdemServico, setIniciarOrdemServico, incicarOrdemServico }) => {
	...
	return (
		<>
			{!dataGrupo &&
				<Modal noBtnCancelar={true} noBtnConcluir={true} handleConcluir={() => null} title={'Iniciar OrdemServico'} size="xs" propsConcluir={{}} labelConcluir={''} dialogClassName={'modal-os-fullscreen-mobile'} aria-labelledby={'aria-labelledby'} labelCanelar="" show={setShowModalAtualizarOrdemServico} showHide={() => { setShowModalAtualizarOrdemServico(); }}>
					<Load />
				</Modal>
			}
			{dataGrupo &&
