...
import { FORMULARIO_GRUPO_ONE_GET } from '../../../api/endpoints/geral.js'
...
const Visualizar = ({ idRegistro, setIdRegistro, setVisualizarRegistro }) => {
    ...
    const buildDetails = (data) => {
        ...
        const fields = [
            { key: 'id', label: 'Codigo' },
            { key: 'name', label: 'Nome' },
            { key: 'formulario_id', label: 'Formulario' },
            { key: 'nr_ordem', label: 'Ordem' },
    ...
    return (
        <>
            {!dataRegistro &&
                <Modal noBtnCancelar={true} noBtnConcluir={true} handleConcluir={() => null} title={'Visualizar Grupo'} size="xs" propsConcluir={{}} labelConcluir={''} dialogClassName={''} aria-labelledby={'aria-labelledby'} labelCanelar="" show={true} showHide={() => { setShowModalVisualizarRegistro(); }}>
                    <Load />
                </Modal>
            }
