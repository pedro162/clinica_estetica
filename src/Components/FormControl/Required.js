...
const Required = ({ data, url_btn, callback_selected, props_btn_search, label_btn_search, ComponentFilter, componentTitle, ...props }) => {
    ...
    const getServicoById = async (idjServico) => {
        ...
        if (json?.mensagem) {
            registro = json?.mensagem[0]
        }

        if (json?.data && json?.data?.data && json?.data?.data[0]) {
            registro = json?.data?.data[0]
        }
    ...
    const handleBlurCod = async ({ target, ...event }) => {
        ...
        if (json?.mensagem && json?.mensagem[0]) {
            registro = json?.mensagem[0]
        }
    ...
    const handleChangeDescription = async ({ target, ...ev }) => {
        ...
        if (json?.mensagem) {
            registro = json?.mensagem
        }

        if (json?.data && json?.data?.data) {
            registro = json?.data?.data
        }

        if (registro) {
            setFilteredSuggestions(registro)
