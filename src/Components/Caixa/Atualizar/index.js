...
const Atualizar = ({idCaixa, setIdCaixa, callback, atualizarCaixa, setAtualizarCaixa})=>{
	...
	React.useEffect(()=>{
		...
		const getCaixa = async ()=>{
			if(idCaixa > 0){
				const {url, options} = CONTAS_RECEBER_ONE_GET(idCaixa, getToken());
	...
	/*
		atualizarCaixa && 
                <Atualizar setCarregandoDadosCaixa={null} atualizarCaixa={setAtualizarCaixa} idCaixa={clientChoice} setDataCaixa={null} setShowModalCriarCaixa={setShowModalAtualizarCaixa} />
	*/
	//<Pesquisar idCaixa={idCaixa} setDataCaixa={setDataCaixa} setCarregandoDadosCaixa={setCarregando} />
	return(
		<>
			{! dataCaixa &&
