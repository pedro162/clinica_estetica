...
import { FORMULARIO_ITEM_ONE_GET } from '../../../api/endpoints/geral.js'
...
const Visualizar = ({ idRegistro, setIdRegistro, setVisualizarRegistro }) => {
    ...
    const buildDetails = (data) => {
        ...
        const fields = [
            { key: 'id', label: 'Codigo' },
            { key: 'formulario_grupo_id', label: 'Grupo' },
            { key: 'name', label: 'Chave' },
            { key: 'label', label: 'Pergunta' },
            { key: 'type', label: 'Tipo' },
            { key: 'options', label: 'Opcoes' },
            { key: 'default_value', label: 'Valor padrao' },
            { key: 'nr_linha', label: 'Nr linha' },
            { key: 'nr_coluna', label: 'Nr coluna' },
    ...
    return (
        <>
            {!dataRegistro &&
                <Modal noBtnCancelar={true} noBtnConcluir={true} handleConcluir={() => null} title={'Visualizar Item'} size="xs" propsConcluir={{}} labelConcluir={''} dialogClassName={''} aria-labelledby={'aria-labelledby'} labelCanelar="" show={true} showHide={() => { setShowModalVisualizarRegistro(); }}>
                    <Load />
                </Modal>
            }
